  ...
  useEffect(() => {
    const fetchLocalization = async () => {
      try {
        ...
        setMessage({ text: 'Failed to fetch localization settings', type: 'error' });
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        setMessage({ text: 'Localization settings updated successfully!', type: 'success' });
      } else {
        setMessage({ text: 'Failed to update localization settings', type: 'error' });
      }
    } catch (error) {
